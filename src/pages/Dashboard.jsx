...
import { generateInsights } from '../utils/smartInsights'; // <--- Import Logic
import { Lightbulb, AlertTriangle, CheckCircle2, Info } from 'lucide-react'; // <--- Import Icons
...
const Dashboard = () => {
  ...
  const monthlyExpenses = thisMonthTransactions.filter(t => t.type === 'expense' && t.category !== 'Investment').reduce((acc, c) => acc + c.amount, 0);
  ...
  // Helper to get Icon/Color based on insight type
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-8 animate-fade-in bg-gray-50/50 dark:bg-gray-950/20 min-h-screen">
      ...
      <div className={`p-6 rounded-3xl border ${insightStyle.border} bg-white dark:bg-gray-900/60 dark:border-gray-800 shadow-sm flex flex-col md:flex-row items-start md:items-center gap-5 transition-all duration-500 hover:shadow-md`}>
        <div className={`p-3 rounded-2xl ${insightStyle.bg} shadow-lg shadow-gray-200 dark:shadow-none shrink-0`}>
          {insightStyle.icon}
        </div>
        <div>
          <h3 className={`font-bold text-lg ${insightStyle.text} dark:text-white flex items-center gap-2`}>
            {dailyInsight.title}
          </h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-white dark:bg-gray-900/60 dark:border-gray-800 p-6 rounded-3xl shadow-sm border border-gray-100 hover:shadow-xl transition-all duration-300 group">
          <div className="flex items-start justify-between mb-4">
            ...
            <span className="text-[10px] font-bold bg-purple-50 dark:bg-purple-900/20 text-purple-700 dark:text-purple-400 px-2 py-1 rounded-lg">Assets</span>
          </div>
          <p className="text-gray-400 dark:text-gray-500 text-xs font-bold uppercase tracking-wider">Total Invested</p>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-white mt-1">{formatCurrency(investmentBalance)}</h2>
        </div>
      </div>
      ...
      {/* QUICK ADD MODAL - Minimal Dark Mode applied for safety */}
      {showForm && (
