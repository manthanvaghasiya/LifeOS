      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 hover:shadow-xl transition-all duration-300 group">
          <div className="flex items-start justify-between mb-4">
             ...
             <span className="text-[10px] font-bold bg-green-50 text-green-700 px-2 py-1 rounded-lg">This Month</span>
          </div>
        ...
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 hover:shadow-xl transition-all duration-300 group">
          <div className="flex items-start justify-between mb-4">
             ...
             <span className="text-[10px] font-bold bg-red-50 text-red-700 px-2 py-1 rounded-lg">This Month</span>
          </div>
        ...
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 hover:shadow-xl transition-all duration-300 group">
          <div className="flex items-start justify-between mb-4">
             ...
             <span className="text-[10px] font-bold bg-purple-50 text-purple-700 px-2 py-1 rounded-lg">Assets</span>
          </div>
          <p className="text-gray-400 text-xs font-bold uppercase tracking-wider">Total Invested</p>
          <h2 className="text-2xl font-bold text-gray-900 mt-1">{formatCurrency(investmentBalance)}</h2>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
          ...
          <div className="bg-white rounded-3xl shadow-lg shadow-gray-200/50 border border-gray-100 flex flex-col h-full min-h-[450px] relative overflow-hidden">
              ...
              <div className="p-6 border-b border-gray-50 flex justify-between items-center bg-white/50 backdrop-blur-sm">
                  <h3 className="text-lg font-bold text-gray-900 flex items-center gap-3">
                      <div className="p-2 bg-green-100 rounded-xl text-green-600"><CheckCircle className="w-5 h-5" /></div> Daily Rituals
                  </h3>
                      ...
                      incompleteHabits.map(habit => (
                          <div key={habit._id} className="group flex items-center justify-between p-4 bg-white border border-gray-100 hover:border-green-200 hover:shadow-lg rounded-2xl transition-all duration-300 cursor-pointer transform hover:-translate-y-0.5" onClick={() => handleToggleHabit(habit._id)}>
                              <div className="flex items-center gap-4">
                                  <div className="w-6 h-6 rounded-full border-2 border-gray-300 group-hover:border-green-500 flex items-center justify-center transition-colors"><div className="w-3 h-3 rounded-full bg-green-500 opacity-0 group-hover:opacity-100 transition-opacity"></div></div>
                                  <span className="font-medium text-gray-700 text-sm group-hover:text-gray-900">{habit.title}</span>
                              </div>
                              <span className="text-xs font-medium text-gray-400 bg-gray-50 px-2 py-1 rounded-lg group-hover:text-green-600 group-hover:bg-green-50 transition-colors">{habit.target} days</span>
                          </div>
                      ))
                  ) : (
                      <div className="py-16 text-center"><div className="w-20 h-20 bg-green-50 rounded-full flex items-center justify-center mx-auto mb-4 shadow-inner"><CheckCircle className="w-10 h-10 text-green-500" /></div><h4 className="text-lg font-bold text-gray-900">All Done For Today!</h4></div>
                  )}
              </div>
          </div>
          ...
          <div className="flex flex-col gap-8">
              ...
              <div className="bg-white rounded-3xl shadow-lg shadow-gray-200/50 border border-gray-100 flex flex-col flex-1 relative overflow-hidden">
                  ...
                  <div className="p-6 border-b border-gray-50 flex justify-between items-center">
                      <h3 className="text-lg font-bold text-gray-900 flex items-center gap-3"><div className="p-2 bg-purple-100 rounded-xl text-purple-600"><Clock className="w-5 h-5" /></div> Today's Spending</h3>
                  </div>
                  <div className="p-0">
                              ...
                              {todayTransactions.map(t => (
                                  <li key={t._id} className="p-5 flex justify-between items-center hover:bg-gray-50 transition-colors cursor-default">
                                      <div className="flex items-center gap-4">
                                          ...
                                          <div><p className="font-medium text-sm text-gray-800">{t.title}</p><div className="flex items-center gap-2 mt-0.5"><span className="text-[10px] font-medium bg-gray-100 text-gray-500 px-1.5 py-0.5 rounded">{t.category}</span></div></div>
                                      </div>
                                      <span className={`font-bold text-sm ${t.type === 'income' ? 'text-green-600' : 'text-gray-900'}`}>{t.type === 'income' ? '+' : '-'} {formatCurrency(t.amount)}</span>
                                  </li>
                              ))}
                          </ul>
                      ) : (
      ...
      {showForm && (
        <div className="fixed inset-0 bg-gray-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fadeIn">
            <div className="bg-white p-8 rounded-[2rem] w-full max-w-sm shadow-2xl relative transform transition-all scale-100">
                ...
                <h3 className="font-bold text-2xl mb-1 text-gray-900">Quick Spend</h3>
                <p className="text-gray-500 text-sm mb-6">Track it before you forget it.</p>
                <form onSubmit={handleQuickAdd} className="space-y-4">
                    <input type="text" placeholder="What is it?" required className="w-full p-4 border border-gray-200 rounded-2xl outline-none focus:ring-2 focus:ring-black bg-gray-50 font-medium" value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} />
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <select className="w-full p-4 border border-gray-200 rounded-2xl outline-none focus:ring-2 focus:ring-black bg-gray-50 font-medium appearance-none" value={formData.paymentMode} onChange={e => setFormData({...formData, paymentMode: e.target.value})}><option value="Bank">Bank</option><option value="Cash">Cash</option></select>
                    </div>
                    <select className="w-full p-4 border border-gray-200 rounded-2xl outline-none focus:ring-2 focus:ring-black bg-gray-50 font-medium appearance-none" value={formData.category} onChange={e => setFormData({...formData, category: e.target.value})}>{DEFAULT_CATEGORIES.map(cat => <option key={cat} value={cat}>{cat}</option>)}<option value="Other">Other</option></select>
                    {formData.category === 'Other' && <input type="text" placeholder="Type Category Name" required className="w-full p-4 border border-blue-200 bg-blue-50 rounded-2xl outline-none focus:ring-2 focus:ring-blue-500 text-blue-800 font-bold" value={customCategory} onChange={e => setCustomCategory(e.target.value)} />}
                    ...
                    <button type="submit" className="w-full bg-gray-900 text-white p-4 rounded-2xl font-bold hover:bg-black shadow-xl hover:shadow-2xl transition-all transform hover:-translate-y-1 mt-4">Save Transaction</button>
                </form>
            </div>
        </div>
      )}
