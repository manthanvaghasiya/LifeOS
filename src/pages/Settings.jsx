...
import {
    ...
    ArrowLeft, ChevronRight // 👈 Add these
} from 'lucide-react';
...
const Settings = () => {
    ...
    const handleImageChange = (e) => {
        ...
        if (file) {
            // 1. Create a fake local URL to show the image immediately
            const objectUrl = URL.createObjectURL(file);
            setImagePreview(objectUrl);

            // 2. TODO: Add logic here to send 'file' to your backend 
            // e.g., setFormData({ ...formData, avatar: file })
        }
    };
    ...
    // Sync state with context if user changes (e.g. re-fetch)
    useEffect(() => {
        if (user) {
            setFormData(prev => ({
                ...prev,
    ...
    const NavItem = ({ id, icon: Icon, label, isDanger }) => (
        <button
            onClick={() => {
                setActiveTab(id);
                setShowMobileMenu(false); // 👈 Hide menu, show content on mobile
            }}
            className={`
    ...
    `}
        >
            ...
            {/* 👈 Arrow icon for mobile drill-down feel */}
    ...
    <div className="max-w-6xl mx-auto px-4 lg:px-8 py-6 lg:py-10 animate-fade-in pb-24 lg:pb-12">
      ...
      {/* --- MOBILE NAVIGATION HEADER (Sticky & Glassy) --- */}
      {/* Only visible when reading content on mobile */}
      ...
      {/* --- PROFILE HEADER --- */}
      {/* Hidden on mobile when viewing content details */}
      ...
      <div className="flex flex-col lg:flex-row gap-8 items-start">
        ...
        {/* --- SIDEBAR MENU --- */}
        {/* Hidden on mobile when viewing content */}
        ...
        {/* --- CONTENT AREA --- */}
        {/* Hidden on mobile when viewing menu list */}
