...
const HabitGrid = ({ habits, weekDays, today, handlePrevWeek, handleNextWeek, toggleHabitDate, handleEdit, deleteHabit }) => {
  ...
  return (
    <div className="bg-white dark:bg-gray-900/60 rounded-[2rem] shadow-xl shadow-gray-200/50 dark:shadow-none border border-gray-100 dark:border-gray-800 overflow-hidden">
      
      {/* Header */}
      <div className="p-6 border-b border-gray-50 dark:border-gray-800 flex justify-between items-center bg-white dark:bg-transparent">
          <h3 className="font-bold text-gray-900 dark:text-white text-lg">Weekly Grid</h3>
          <div className="flex items-center gap-4 bg-gray-50 dark:bg-gray-800 px-2 py-1 rounded-full border border-gray-100 dark:border-gray-700">
          ...
          <table className="w-full text-left border-collapse">
              <thead>
                  <tr className="border-b border-gray-50 dark:border-gray-800 text-gray-400 dark:text-gray-500 text-xs font-bold uppercase tracking-wider">
                      ...
                      {weekDays.map(date => (
                          <th key={date} className={`p-3 text-center min-w-[50px] ${date === today ? 'text-indigo-600 dark:text-indigo-400' : ''}`}>
