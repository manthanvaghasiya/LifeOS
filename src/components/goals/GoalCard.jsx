...
const GoalCard = ({ goal, handleEdit, handleDelete, handleToggle }) => {
  // FIX: Reset time to midnight to compare DATES only
  ...
  return (
    <div className={`p-4 rounded-xl border-2 shadow-sm flex flex-col justify-between h-full transition hover:shadow-md ${borderColor} ${bgColor}`}>
      ...
      <button onClick={() => handleToggle(goal._id)} className={`w-full py-1.5 rounded-lg text-xs font-bold flex items-center justify-center gap-1 transition border ${isCompleted ? 'bg-white border-green-200 text-green-700 hover:bg-gray-50' : (isOverdue ? 'bg-red-600 border-red-600 text-white hover:bg-red-700' : 'bg-white border-gray-300 text-gray-700 hover:bg-gray-50')}`}>
